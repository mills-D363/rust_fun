...
fn unicode_to_utf8_2bytes(point_u16: u16) -> (u8, u8) {
    let (mask_1, mask_2, mask_3) = (0x0f00u16, 0x00f0u16, 0x000fu16);
    let (base_b1, base_b2) = (0xc0u8, 0x80u8);

    let b1_p1: u8 = ((mask_1 & point_u16) >> 6) as u8;
    let b1_p2: u8 = ((mask_2 & point_u16) >> 6) as u8;

    let b2_p1: u8 = ((mask_2 & point_u16) as u8) & 0x30;
    let b2_p2: u8 = (mask_3 & point_u16) as u8;
...
fn unicode_to_utf8_3bytes(point_u16: u16) -> (u8, u8, u8) {
    let (mask_1, mask_2, mask_3, mask_4) = (0xf000u16, 0x0f00u16, 0x00f0u16, 0x000fu16);
    let (base_b1, base_b2, base_b3) = (0xe0u8, 0x80u8, 0x80u8);

    let b1_p1: u8 = ((mask_1 & point_u16) >> 12) as u8;

    let b2_p1: u8 = ((mask_2 & point_u16) >> 6) as u8;
    let b2_p2: u8 = ((mask_3 & point_u16) >> 6) as u8;

    let b3_p1: u8 = ((mask_3 & point_u16) as u8) & 0x30;
    let b3_p2: u8 = (mask_4 & point_u16) as u8;
                    ...
                    match val_hex {
                        0x0000..=0x007F => {
                            ...
                            println!(
                                "{BRNB}U+{:04x} ({}) ↔ {:#04x}{NC}",
                                v_u32,
                                char::from_u32(v_u32).unwrap_or('�'),
                                v_u32
                            );
                        }
                        0x0080..=0x07FF => {
                            print_conversion_2bytes();
                            let v_u16: u16 = val_hex as u16;
                            let (b1, b2) = unicode_to_utf8_2bytes(v_u16);
                            println!(
                                "{BRNB}U+{:04x} ({}) ↔ {:#04x} {:#04x}{NC}",
                                v_u16,
                                char::from_u32(v_u16 as u32).unwrap_or('�'),
                                b1,
                                b2
                            );
                        }
                        0x0800..=0xFFFF => {
                            ...
                            let v_u16: u16 = val_hex as u16;
                            let (b1, b2, b3) = unicode_to_utf8_3bytes(v_u16);
                            println!(
                                "{BRNB}U+{:04x} ({}) ↔ {:#04x} {:#04x} {:#04x}{NC}",
                                v_u16,
                                char::from_u32(v_u16 as u32).unwrap_or('�'),
                                b1,
                                b2,
