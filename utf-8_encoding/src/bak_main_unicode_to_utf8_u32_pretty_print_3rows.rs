...
fn pretty_print_1byte(point_u32: u32) {
    ...
    match point_u32 {
        ...
        _ => {
            ...
            println!("{BRNB}{: ^6}{NC}", format!("'{}'",char::from_u32(point_u32).unwrap_or('�')));
        }
    }
}
...
fn pretty_print_2bytes(point_u32: u32, byte1: u8, byte2: u8) {
    ...
    match point_u32 {
        ...
        _ => {
            ...
            println!("{BRNB}{} {: <8}  {: <8}{NC}", " ".repeat(8), format!("{:#04x}", byte1), format!("{:#04x}", byte2));
            println!("{BRNB}{: ^6}{NC}", format!("'{}'",char::from_u32(point_u32).unwrap_or('�')));
        }
    }
}
...
fn pretty_print_3bytes(point_u32: u32, byte1: u8, byte2: u8, byte3: u8) {
    ...
    println!("{BRNB}{} {: <8}  {: <8}  {: <8}{NC}", " ".repeat(8), format!("{:#04x}", byte1), format!("{:#04x}", byte2), format!("{:#04x}", byte3));
    println!("{BRNB}{: ^6}{NC}", format!("'{}'",char::from_u32(point_u32).unwrap_or('�')));
}
...
fn pretty_print_4bytes(point_u32: u32, byte1: u8, byte2: u8, byte3: u8, byte4: u8) {
    ...
    println!("{BRNB}{} {: <8}  {: <8}  {: <8}  {: <8}{NC}", " ".repeat(10), format!("{:#04x}", byte1), format!("{:#04x}", byte2), format!("{:#04x}", byte3), format!("{:#04x}", byte4));
    println!("{BRNB}{: ^8}{NC}", format!("'{}'",char::from_u32(point_u32).unwrap_or('�')));
}
